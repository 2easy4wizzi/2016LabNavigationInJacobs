...
private:
    ...
    int _classes[NUMBER_OF_NEIGBHORS];
    string _videoPath;

public:
    Node(string name, string number, int floor, neighborPair (&neighbors)[NUM_OF_NEIGBHORS], int (&classes)[NUMBER_OF_NEIGBHORS], int videoStartIndex, int videoEndIndex, string videoPath);
    //Node(Node* other);
    ~Node();
